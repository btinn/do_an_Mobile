...
class _ManHinhHoSoNguoiDungState extends State<ManHinhHoSoNguoiDung>
  ...
  Future<void> _taiThongTinNguoiDung() async {
    ...
    try {
      // 1. Lấy dữ liệu người dùng từ Realtime Database (auth_users)
      final authUserRef =
          FirebaseDatabase.instance.ref('auth_users/${widget.maNguoiDung}');
      ...
      if (authUserSnapshot.exists) {
        final authUserData =
            Map<String, dynamic>.from(authUserSnapshot.value as Map);
        displayName = authUserData['displayName'] ?? displayName;
        photoURL = authUserData['photoURL'] ?? photoURL;
      }

      // 2. Lấy danh sách theo dõi từ Firestore
      final firestoreDoc = await FirebaseFirestore.instance
          .collection('user')
          .doc(widget.maNguoiDung)
          .get();
      ...
      if (firestoreDoc.exists) {
        ...
        followers = data['soLuongNguoiTheoDoi'] ?? [];
        following = data['soLuongDangTheoDoi'] ?? [];

        // Kiểm tra nếu người hiện tại đã theo dõi thì cập nhật _dangTheoDoi
        final nguoiDungHienTai =
            context.read<DangKiDangNhapEmail>().nguoiDungHienTai;
        if (nguoiDungHienTai != null) {
          _dangTheoDoi = followers.contains(nguoiDungHienTai.ma);
        }
      }

      // 3. Cập nhật thông tin UI
      setState(() {
        _thongTinNguoiDung = {
  ...
  Future<void> _taiDanhSachCongThucCuaTacGia() async {
    try {
      ...
      _danhSachCongThucCuaTacGia =
          await _dichVuCongThuc.layDanhSachCongThucCuaTacGia(widget.maNguoiDung,
              widget.maNguoiDung // Có thể thay bằng UID của người đang xem
              );
  ...
  void _toggleTheoDoi() async {
    ...
    try {
      if (_dangTheoDoi) {
        // Huỷ theo dõi
        await userRef.doc(uidNguoiDangXem).update({
          'soLuongNguoiTheoDoi': FieldValue.arrayRemove([uidNguoiHienTai])
        });
        await userRef.doc(uidNguoiHienTai).update({
          'soLuongDangTheoDoi': FieldValue.arrayRemove([uidNguoiDangXem])
        });
      } else {
        // Bắt đầu theo dõi
        await userRef.doc(uidNguoiDangXem).update({
          'soLuongNguoiTheoDoi': FieldValue.arrayUnion([uidNguoiHienTai])
        });
        await userRef.doc(uidNguoiHienTai).update({
          'soLuongDangTheoDoi': FieldValue.arrayUnion([uidNguoiDangXem])
        });
      }
            ...
            child: Column(
              children: [
                ...
                Row(
                  ...
                  children: [
                    _xayDungThongKe(
                      _danhSachCongThucCuaTacGia.length.toString(),
                      'Công Thức',
                    ),
                    Container(
                      height: 30,
